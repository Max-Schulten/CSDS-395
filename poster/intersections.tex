...
\logoright{\includegraphics[height=0.8cm]{assets/black_cw.png}}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{References and Relevant Classes}
      ...
      CSDS 132, CSDS 133, CSDS 221, CSDS 233, CSDS 293, CSDS 310, CSDS 338, CSDS 340, CSDS 356, CSDS 391
...
\end{figure}
  ...
  \begin{block}{Project Challenges}
  	\begin{itemize}
  		\item Resume Classification: Sparse representations (TF-IDF, BoW) fail to capture semantic nuance in resume text; we instead used dense embeddings, yielding acceptable classification accuracy.
  		\item Skill Extraction: We matched skills deterministically against the ESCO and O*NET databases, with a GLiNER model and embeddings handling fuzzy matches (e.g.\ Cloud infrastructure \& AWS).
  		\item Composite Scoring: Scoring components lacked a shared $[0,1]$ co-domain; empirical calibration at each step produced interpretable outputs.
  	\end{itemize}
...
\begin{column}{\colwidth}
  ...
  \begin{exampleblock}{Matching Model}
    ...
    \begin{itemize}
        ...
        Matches deterministically from databases aggregated from O*NET and ESCO alongside a fuzzy matching system leveraging {\tt all-MiniLM-L6-v2} embeddings and cosine similarity:
        \begin{equation}
  ...
  \begin{block}{Testing and Results}
    ...
    See the front end example in figure \ref{fig:frontend}, and below, which describes the approximate distribution of our pipeline for randomly paired jobs and resumes.

    \begin{itemize}
        \item \textbf{Score Distribution:} Total scores across 25k+ randomly paired job, resume combinations follow a somewhat normal distribution centered around 0.35, confirming the pipeline discriminates meaningfully rather than saturating at extremes. This center is expected as random to random matches are more likely to be poor matches when comparing across industries.
        \item \textbf{Scoring Correlates:} Skill coverage and semantic similarity are positively correlated, validating that both signals capture complementary but consistent notions of fit. Education alignment shows lower correlation with the other two components, as expected.
    \end{itemize}
